...
def buildBridge(level, startPoint, endPoint, bridgeY, width, biomeId):
	...
	if offsetInterval >= 2:
		...
		placeOrthogonalBridgeHead(level, startPoint[0], bridgeY, startPoint[1], width, headLength) # Start of bridge
		placeOrthogonalBridgeHead(level, endPoint[0], bridgeY, endPoint[1], width, headLength, True) # End of bridge

		# Offsets points of bridge to edge of platform.
		startPoint = (startPoint[0] + (headLength - 1) * bridgeMainDirectionX, startPoint[1] + (headLength - 1) * bridgeMainDirectionZ)
		endPoint = (endPoint[0] + (headLength - 1) * bridgeMainDirectionX * -1, endPoint[1] + (headLength - 1) * bridgeMainDirectionZ * -1)
		...
		diagonal = True
		tempWidth = width / 2
		placeDiagonalBridgeHead(level, startPoint[0], bridgeY, startPoint[1], width) # Start of bridge
		placeDiagonalBridgeHead(level, endPoint[0], bridgeY, endPoint[1], width, True) # End of bridge

		offsetMain = tempWidth
		offsetSecondaryStart = tempWidth - 2
		offsetSecondaryEnd = tempWidth + int(tempWidth / offsetInterval)
		startPoint = (startPoint[0] + offsetMain*bridgeMainDirectionX + offsetSecondaryStart*bridgeSecondaryDirectionX, startPoint[1] + offsetMain * bridgeMainDirectionZ + offsetSecondaryStart*bridgeSecondaryDirectionZ)
		endPoint = (endPoint[0] + (offsetMain*bridgeMainDirectionX + offsetSecondaryEnd*bridgeSecondaryDirectionX) * -1, endPoint[1] + (offsetMain * bridgeMainDirectionZ + offsetSecondaryEnd*bridgeSecondaryDirectionZ) * -1)
	...
	for i in range(1, newBridgeLength - 1):
		...
		if (i + 1) % offsetInterval < 1:
			if offsetNext:
				placeBridgeSection(level, x, bridgeY, z, width + 2, materials["upper slab"], True, True)
				currentOffset += 1
			else:
				placeBridgeSection(level, x, bridgeY, z, width + 1, materials["upper slab"], False, True)
			offsetNext = True
		elif offsetNext:
			placeBridgeSection(level, x, bridgeY, z, width + 1, materials["upper slab"], True, False)
			offsetNext = False
			currentOffset += 1
		else:
			placeBridgeSection(level, x, bridgeY, z, width, materials["upper slab"], False, False)

def placeBridgeSection(level, x, y, z, width, bridgeMaterial, extraFenceLeft = False, extraFenceRight = False, placeFence = True):
	# Place main part of bridge
	for i in range(width):
		setBlock(level, x + i * bridgeSecondaryDirectionX, y, z + i * bridgeSecondaryDirectionZ, bridgeMaterial[0], bridgeMaterial[1])

	# Place fencing
	if placeFence:
		setBlock(level, x, y + 1, z, materials["fence"])
		setBlock(level, x + (width - 1) * bridgeSecondaryDirectionX, y + 1, z + (width - 1) * bridgeSecondaryDirectionZ, materials["fence"])
		if extraFenceLeft:
			setBlock(level, x + bridgeSecondaryDirectionX, y + 1, z + bridgeSecondaryDirectionZ, materials["fence"])
		if extraFenceRight:
			setBlock(level, x + (width - 2) * bridgeSecondaryDirectionX, y + 1, z + (width - 2) * bridgeSecondaryDirectionZ, materials["fence"])

def placeOrthogonalBridgeHead(level, x, y, z, width, headLength, flipped = False):
	placePillarWithTorch(level, x, y, z)

	for i in range(1, width-1):
		setBlock(level, x + i * bridgeSecondaryDirectionX, y, z + i * bridgeSecondaryDirectionZ, materials["lower slab"], materials["lower slab"])

	placePillarWithTorch(level, x + (width - 1) * bridgeSecondaryDirectionX, y, z + (width - 1) * bridgeSecondaryDirectionZ)
...
def placeDiagonalBridgeHead(level, x, y, z, width, flipped = False):
	...
	placePillarWithTorch(level, x + flipped * 2 * (bridgeMainDirectionX + bridgeSecondaryDirectionX), y, z + flipped * -1 * (bridgeMainDirectionZ + bridgeSecondaryDirectionZ))
	placePillarWithTorch(level, x + flipped * -1 * (bridgeMainDirectionX + bridgeSecondaryDirectionX), y, z + flipped * 2 * (bridgeMainDirectionZ + bridgeSecondaryDirectionZ))

	# Steps
	setBlock(level, x + flipped * -1 * (bridgeMainDirectionX + bridgeSecondaryDirectionX), y, z + flipped * (bridgeMainDirectionZ + bridgeSecondaryDirectionZ), materials["lower slab"])
	for i in range(tempWidth):
		for j in range(2):
			setBlock(level, x + i * flipped * (bridgeMainDirectionX + bridgeSecondaryDirectionX), y, z + j * flipped * (bridgeMainDirectionZ + bridgeSecondaryDirectionZ) - i * flipped, materials["lower slab"])
	...
	for i in range(2 * tempWidth - 1):
		tempX = x + flipped * (i * bridgeMainDirectionX + offsetSecondary * bridgeSecondaryDirectionX)
		tempZ = z + flipped * (i * bridgeMainDirectionZ + offsetSecondary * bridgeSecondaryDirectionZ)
		for j in range(i, -1, -1):
			setBlock(level, tempX + flipped * (-1 * j * bridgeSecondaryDirectionX + offsetMain * bridgeMainDirectionX), y, tempZ + flipped * (-1 * j * bridgeSecondaryDirectionZ + offsetMain * bridgeMainDirectionZ), materials["upper slab"])
	...
	for i in range(width-1):
		...
		for j in range(flapSize):
			if j == flapSize - 1:
				setBlock(level, tempX + flipped * (j * bridgeSecondaryDirectionX + offsetMain * bridgeMainDirectionX), y + 1, tempZ + flipped * (j * bridgeSecondaryDirectionZ + offsetMain * bridgeMainDirectionZ), materials["fence"])
				if extraFence:
					setBlock(level, tempX + flipped * ((j - 1) * bridgeSecondaryDirectionX + offsetMain * bridgeMainDirectionX), y + 1, tempZ + flipped * ((j - 1) * bridgeSecondaryDirectionZ + offsetMain * bridgeMainDirectionZ), materials["fence"])
					extraFence = False
			setBlock(level, tempX + flipped * (j * bridgeSecondaryDirectionX + offsetMain * bridgeMainDirectionX), y, tempZ + flipped * (j * bridgeSecondaryDirectionZ + offsetMain * bridgeMainDirectionZ), materials["upper slab"])
...
def buildSmallBridge(level, startPoint, endPoint, bridgeY, width):
	...
	for i in range(bridgeLength):
		...
		if i % offsetInterval < 1 and (i != 0 or offsetInterval == 1):
			placeBridgeSection(level, x, bridgeY, z, width + 1, materials["lower slab"], False, False, False)
			currentOffset += 1
		else:
			placeBridgeSection(level, x, bridgeY, z, width, materials["lower slab"], False, False, False)
...
def buildDocks(level, startPoint, endPoint, bridgeY, width, length, blocks):
	...
	# Places docks.
	placeDock(level, startPoint[0], bridgeY, startPoint[1], width, dockLength, blocks) # First dock
	placeDock(level, endPoint[0], bridgeY, endPoint[1], width, dockLength, blocks, True) # Second dock
...
def placeDock(level, x, y, z, width, dockLength, blocks, flipped = False):
	...
	for i in rangeList:
		sectionX = x + i * bridgeMainDirectionX
		sectionZ = z + i * bridgeMainDirectionZ
		placeBridgeSection(level, sectionX, y, sectionZ, width, materials["upper slab"], False, False, False)
